...
export async function processNextBatch(): Promise<ProcessBatchResult> {
  ...
  try {
    ...
    // ── 5b. n8n brain (optional) ─────────────────────────────────────────────
    // When the workspace has an enabled `n8n` integration, an external n8n
    // workflow owns reply generation + media understanding. decide() above
    // stays the gate (handoff / ai_enabled / rate limits) and dispatchText()
    // below stays the single exit point, so the 24h-window & opt-out guards,
    // persistence and the inbox all keep working. Falls through to the internal
    // engine when no n8n integration is configured.
    const n8nIntegration = await getN8nIntegration(batch.workspace_id);
    if (n8nIntegration) {
      const n8nReply = await callN8nAgent({
...
// ──────────────────────────────────────────────────────────────────────────────
// applyN8nActions (private)
// Maps the optional structured actions returned by the n8n workflow onto the
// app's existing primitives. Each action is best-effort and isolated: a failing
// action is logged but never breaks the turn (the reply was already dispatched).
//   handoff        → hand the conversation to a human (ai_enabled=false)
//   add_tag        → merge a tag onto the contact
...
async function applyN8nActions(p: ApplyN8nActionsParams): Promise<void> {
  for (const action of p.actions) {
    try {
      switch (action.type) {
        ...
        case "add_tag": {
          const tag =
            typeof action.tag === "string" ? action.tag.trim() : "";
          if (!tag) break;
          const { data: c } = await p.supabase
            .from("contacts")
            .select("tags")
            .eq("id", p.contactId)
            .maybeSingle();
          ...
          await p.supabase
            .from("contacts")
            .update({ tags: Array.from(new Set([...existing, tag])) })
            .eq("id", p.contactId);
          break;
        }
        ...
        case "send_template": {
          const name =
            typeof action.template_name === "string"
          ...
          await dispatchTemplate({
            ...
            templateName: name,
            templateLanguage:
              typeof action.language === "string" ? action.language : "es",
          });
          break;
        }
        ...
        case "set_stage": {
          const stage =
            typeof action.stage === "string" ? action.stage.trim() : "";
          if (!stage) break;
          await p.supabase
            .from("contacts")
            .update({ stage })
